---
title: 'Airbnb: Understanding the Data'
...
# Describe the source
  ...
  There did not seem to be much limitations of the source beyond there being some missing varirables. The variable names
...
We see that there are 16 attributes and 1509 observations in the original dataset.
```{r explanation = FALSE}
...
Host contains the host's id, host_name, and listings_count. Host_id is a continuous variable which contains a unqiue number that represents the host, host_name is a categorical variable, and listings_count is a discrete variable which houses the amount of listings a host has uploaded.
```{r host}
host <- tibble (
...
The listing table contains the listing_id (continuous), name (categorical), price of the listing (discrete), neighbourhood_group (categorical), neighbourhood (categorical), latitude (continuous), longitude (continuous), room_type (categorical), and the
minimum amount of nights a guest can stay (discrete). After we create the table we are going to change neighbourhood_group, neighbourhood, and room_type to factors or discrete data. By creating a graph, we can see that the neighbourhood with the most listings are Brooklyn and Manhatten.
```{r listing} 
listing <- tibble (
  listing_id = listings$id,
  name = listings$name,
